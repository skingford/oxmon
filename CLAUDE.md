...
- **Instance contact routing**: Instance contacts are stored in `instance_contacts` table, allowing per-instance responsible person configuration. Each contact has `agent_patterns` (JSON array of glob patterns), `contact_name`, and optional contact methods: `contact_email`, `contact_phone`, `contact_dingtalk` (webhook URL), `contact_webhook`. When an alert fires, `NotificationManager.send_to_channels()` first queries matching contacts via `find_contacts_for_agent(agent_id)`. If contacts exist, notifications are routed only to their configured contact methods (email→email channels, phone→sms channels, dingtalk→dingtalk channels, webhook→webhook/weixin channels). If no contacts match, falls back to default channel recipients. Managed via REST API (`/v1/instance-contacts`).
...
- **Cloud monitoring**: Server-side scheduler collects metrics from cloud providers (Tencent Cloud, Alibaba Cloud, Sangfor SCP) via their native APIs. Cloud accounts are stored in a dedicated `cloud_accounts` table (NOT in `system_configs`). Table schema includes fields: `id`, `config_key` (unique), `provider`, `display_name`, `description`, `account_name`（云账号名称如"主账号"）, `secret_id` (AccessKey ID), `secret_key` (AccessKey Secret), `regions` (JSON array of region strings, e.g. `["ap-shanghai","ap-guangzhou"]`), `endpoint` (optional, private cloud host for Sangfor SCP), `region_for_sign` (optional, AWS4 signing region for Sangfor SCP, default `regionOne`), `scp_auth_token` (optional, Cookie auth token for Sangfor SCP 6.3.0 and earlier), `collection_interval_secs`, `enabled`, `created_at`, `updated_at`. Managed exclusively via REST API (`/v1/cloud/accounts`). Batch import supported via `POST /v1/cloud/accounts/batch` with pipe-delimited text format: `账号名:SecretId:SecretKey:region1,region2|...`. Supported providers: `tencent` (腾讯云), `alibaba` (阿里云), `sangfor` (深信服私有云 SCP). Cloud instances are discovered via provider APIs and stored in `cloud_instances` table. Collected metrics are written to the same time-partitioned storage as agent metrics. Virtual agent IDs follow format `cloud:{provider}:{instance_id}` (e.g., `cloud:tencent:ins-abc123`), enabling alert rules to target cloud instances using glob patterns. Metrics include `cloud.cpu.usage`, `cloud.memory.usage`, `cloud.disk.usage`. API signing implemented without OpenSSL: TC3-HMAC-SHA256 for Tencent, ACS v1 HMAC-SHA1 for Alibaba, AWS4-HMAC-SHA256 for Sangfor SCP (service=`sdk-api`, region=`regionOne` by default, signed_headers=`cookie;x-amz-date`; SCP 6.3.0 and earlier require `scp_auth_token` in account config to send `Cookie: aCMPAuthToken=<token>`). Sangfor SCP disables TLS cert verification (self-signed). Rate limiting and exponential backoff retry for Alibaba Cloud (10 req/s with token bucket). Scheduler is in `oxmon-server/src/cloud/scheduler.rs`, API routes in `oxmon-server/src/cloud/api.rs`. Table is auto-created on first startup. If migrating from old `system_configs` storage, data is automatically migrated and old entries are deleted. Default placeholder accounts for all three providers are auto-seeded on first startup (disabled, empty credentials) via `cloud_seed::init_default_cloud_accounts()`.

- **AI account management**: AI accounts (for AI-powered report generation) are stored in a dedicated `ai_accounts` table (NOT in `system_configs`). Table schema includes fields: `id`, `config_key` (unique), `provider`, `display_name`, `description`, `api_key`, `model`, `base_url`, `api_mode`, `timeout_secs`, `max_tokens`, `temperature`, `collection_interval_secs`, `enabled`, `created_at`, `updated_at`. Managed exclusively via REST API (`/v1/ai/accounts`). Supported providers: `zhipu` (智谱 GLM-4/GLM-5), `kimi`, `minimax`, `claude`, `codex`, `custom`. The AI scheduler (`oxmon-server/src/ai/scheduler.rs`) uses `CertStore::list_ai_accounts()` to load accounts and `build_analyzer()` to create provider-specific analyzers. For data migration from old `system_configs` storage, use `scripts/migrate_ai_accounts.sh`. For verification, use `scripts/verify_ai_accounts.sh`.
...
```
Cloud accounts are **not** configured in TOML — use `/v1/cloud/accounts` REST API exclusively. Cloud accounts are stored in the `cloud_accounts` table with credentials and per-account `collection_interval_secs`. Supported providers: `tencent` (腾讯云), `alibaba` (阿里云), `sangfor` (深信服私有云 SCP，需额外填写 `endpoint` 字段). Cloud instances are represented as virtual agents with ID format `cloud:{provider}:{instance_id}`.
