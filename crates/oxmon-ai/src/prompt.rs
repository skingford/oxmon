...
fn format_metrics(current: &[MetricSnapshot], history: &[HistoryMetric]) -> String {
    ...
    output.push_str("| # | Agent ID | 实例名称 | 类型 | CPU(%) | 内存(%) | 磁盘(%) | CPU均值 | 内存均值 | 磁盘均值 | 风险 |\n");
    output.push_str("|---|----------|----------|------|--------|---------|---------|---------|----------|----------|------|\n");
...
- **实例 ID**：`agent-xxx`（实例名称：xxx，若表格中"实例名称"列不为"-"则填写，否则省略此括号）
...
- **Instance ID**: `agent-xxx` (Name: xxx — include if the "实例名称" column is not "-", otherwise omit)
