...
impl MetricService for MetricServiceImpl {
    async fn report_metrics(
        ...
        if let Some(ref si) = proto.system_info {
            let hostname = if si.hostname.is_empty() {
                None
            } else {
                Some(si.hostname.as_str())
            };
            let os = if si.os.is_empty() {
                None
            } else {
                Some(si.os.as_str())
            };
            let os_version = if si.os_version.is_empty() {
                None
            } else {
                Some(si.os_version.as_str())
            };
            let arch = if si.arch.is_empty() {
                None
            } else {
                Some(si.arch.as_str())
            };
            let kernel_version = if si.kernel_version.is_empty() {
                None
            } else {
                Some(si.kernel_version.as_str())
            };
            let cpu_cores = if si.cpu_cores > 0 {
                Some(si.cpu_cores)
            } else {
                None
            };
            let memory_gb = if si.memory_gb > 0.0 {
                Some(si.memory_gb)
            } else {
                None
            };
            let disk_gb = if si.disk_gb > 0.0 {
                Some(si.disk_gb)
            } else {
                None
            };
        ...
        // Record agent report log (async, non-blocking)
        {
            ...
            let (hostname, os, os_version, arch, kernel_version, cpu_cores, memory_gb, disk_gb) =
                proto.system_info.as_ref().map_or(
                    ...
                    |si| {
                        (
                            if si.hostname.is_empty() {
                                None
                            } else {
                                Some(si.hostname.clone())
                            },
                            if si.os.is_empty() {
                                None
                            } else {
                            ...
                            if si.os_version.is_empty() {
                                None
                            } else {
                                Some(si.os_version.clone())
                            },
                            if si.arch.is_empty() {
                                None
                            } else {
                            ...
                            if si.kernel_version.is_empty() {
                                None
                            } else {
                                Some(si.kernel_version.clone())
                            },
                            if si.cpu_cores > 0 {
                                Some(si.cpu_cores)
                            } else {
