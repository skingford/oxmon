...
impl Endpoint {
    ...
    /// Sets the max HTTP/2 frame size the client will accept from the server.
    ///
    /// Sent as SETTINGS_MAX_FRAME_SIZE during HTTP/2 handshake, allowing
    /// the server/proxy to send larger frames without triggering FRAME_SIZE_ERROR.
