...
- **WHEN** a POST request is made to `/api/v1/certs/domains` with body `{"domain": "example.com"}`
...
- **WHEN** a POST request is made to `/api/v1/certs/domains` with body `{"domain": "api.example.com", "port": 8443, "check_interval_secs": 3600, "note": "API server"}`
...
- **WHEN** a POST request is made to `/api/v1/certs/domains/batch` with body `{"domains": [{"domain": "a.com"}, {"domain": "b.com", "port": 8443}]}`
...
- **WHEN** a GET request is made to `/api/v1/certs/domains`
...
- **WHEN** a GET request is made to `/api/v1/certs/domains?enabled=true`
...
- **WHEN** a GET request is made to `/api/v1/certs/domains/:id` with a valid id
- **THEN** the API SHALL return the domain record with all fields

#### Scenario: Domain not found
- **WHEN** a GET request is made to `/api/v1/certs/domains/:id` with an unknown id
...
- **WHEN** a PUT request is made to `/api/v1/certs/domains/:id` with body `{"port": 8443}`
- **THEN** the API SHALL update the port to 8443 and return the updated domain object

#### Scenario: Update check interval
- **WHEN** a PUT request is made to `/api/v1/certs/domains/:id` with body `{"check_interval_secs": 7200}`
- **THEN** the API SHALL update the check_interval_secs to 7200 and return the updated domain object

#### Scenario: Disable a domain
- **WHEN** a PUT request is made to `/api/v1/certs/domains/:id` with body `{"enabled": false}`
...
- **WHEN** a PUT request is made to `/api/v1/certs/domains/:id` with body `{"check_interval_secs": null}`
...
- **WHEN** a DELETE request is made to `/api/v1/certs/domains/:id`
- **THEN** the API SHALL remove the domain record and its check results, and return HTTP 204

#### Scenario: Delete non-existent domain
- **WHEN** a DELETE request is made to `/api/v1/certs/domains/:id` with an unknown id
...
- **WHEN** a GET request is made to `/api/v1/certs/status`
...
- **WHEN** a GET request is made to `/api/v1/certs/status/:domain`
...
- **WHEN** a GET request is made to `/api/v1/certs/status/:domain` for a domain that is not being monitored
- **THEN** the API SHALL return HTTP 404
